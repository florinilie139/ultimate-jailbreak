...
public plugin_init()
{
    ...
    ball_distance = register_cvar("jb_ball_distance", "650")
    ...
    register_clcmd("jb_football", "ShowMainMenu")
...
public PlayerPreThink(id)
{
    ...
    if(pev(id, pev_button) & IN_USE && !(pev(id, pev_oldbuttons) & IN_USE) && g_buildingNet[id]) {
        ...
        else
        {
            ...
            ColorChat(id, "[AnNA]Plasa #%d creata cu succes", ++countnets)
        }
    }
...
public HandleBallMenu(id, key)
{
    ...
    switch(key)
    {
        case 0:
        {
            ...
            if(pev_valid(ball))
                ColorChat(id, "[AnNA]Minge creata cu succes")
            else
                ColorChat(id, "[AnNA]Nu se poate crea mingea")
        }
        case 1:
        {
            if(!g_bHighlight[id][1])
            {
                ...
                ColorChat(id, "[AnNA]Ball highlight has been^x04 Enabled^x01.")
            } else {
                ...
                ColorChat(id, "[AnNA]Ball highlight has been^x03 Disabled^x01.")
            }
        }
        case 2:
        {
            ...
            if(bFound)
                ColorChat(id, "[AnNA]Minge stearsa cu succes")
            else
                ColorChat(id, "[AnNA]Mingea nu a putut fi gasita")
        }
        case 9:
        {
            ShowMainMenu(id)
...
public FwdThinkBall(ent) {
    ...
    static iOwner; iOwner = pev(ent, pev_iuser1)
    static iSolid; iSolid = pev(ent, pev_solid)
...
public Goal()
{
    ...
    if(g_Owner != 0)
        show_hudmessage(0, "[AnNA]%s a inscris un gol^nde la distanta de %d unitati!", name, fdistance)
