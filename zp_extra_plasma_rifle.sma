...
public plugin_init() 
{
	...
	g_iItemId = zp_register_extra_item("Plasma Rifle", g_Cost, ZP_TEAM_HUMAN)
...
public Event_NewRound()
{
	...
	{
		if(g_avaible[i]>MAX_ROUNDS || g_avaible[i]<0)
			g_avaible[i]=0;
		if(g_avaible[i]>0)
			g_avaible[i]--;
		g_HasRifle[i] = false
...
{
	if(itemid == g_iItemId && g_avaible[id]==0)
	{		
		g_avaible[id]=MAX_ROUNDS;
