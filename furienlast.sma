...
new const gs_FuriensModel[ ] = "admin_girl1";
new const gs_AntiFuriensModel[ ] = "neo3";
...
{
   new s_Buffer[ 256 ];
   formatex ( s_Buffer, charsmax ( s_Buffer ), "models/player/%s/%s.mdl", gs_FuriensModel, gs_FuriensModel );
   precache_model ( s_Buffer );
   formatex ( s_Buffer, charsmax ( s_Buffer ), "models/player/%s/%s.mdl", gs_AntiFuriensModel, gs_AntiFuriensModel );
   precache_model ( s_Buffer );
   if( !ReadCfgFile() )
   {
...
public plugin_init()
{
   ...
   set_task(1.0,"Check_Players_Cvars2",_,_,_,"b");
...
public Player_Spawn_Post( id )
{
   if( is_user_alive(id) )
   {
      ...
      if( __get_user_team(id) == Furien )
      {
         ...
         cs_set_user_model(id,gs_FuriensModel);
      }
      else
      {
         set_user_footsteps(id, 0)
         set_user_gravity(id, 1.0)
         cs_set_user_model(id,gs_AntiFuriensModel);
...
/* ShowSpeedCvarsMenu(id)
{
   ...
   n += formatex(szMenu[n], charsmax(szMenu)-n, "^ny1w. %L", id, "YES")
   n += formatex(szMenu[n], charsmax(szMenu)-n, "^ny2w. %L", id, "NO")
