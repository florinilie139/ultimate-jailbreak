...
public CmdBan(client, level, cid)
{
	...
	client_print(target,print_chat,"*** %s ai primit ban de la ADMINUL %s ***", target_name , admin_name ) ;
	client_print(target,print_chat,"*** %s daca consideri ca ADMINUL %s a abuzat de aceasta comanda fa o reclamatie aici WwW.Mevid.Ro***", target_name , admin_name ) ;
	client_print(target,print_chat,"*** %s ti-au fost facute 2 ss (poze) in cstrike/(numele hartii )***", target_name) ;
	client_cmd( 0, "spk ^"vox/banned ^"");
	client_cmd(target,"wait;snapshot;wait;snapshot") ;
