...
public plugin_init() 
{
	...
	g_pCvar[3] = register_cvar("ghost_visible", "1")
	...
	formatex(szPluginInfoMsg, sizeof szPluginInfoMsg - 1, "%s %s by %s", PLUGIN, VERSION, AUTHOR)
	formatex(szGhostInfoMsg, sizeof szGhostInfoMsg - 1, "You Are A Ghost Till Round End Have Fun!")
...
{	
	...
	new len = formatex(menu, sizeof menu -1,"\yGhost Menu - by vato loco [GE-S]^n^n")
	len += formatex(menu[len], sizeof menu -1 -len,"\r Do You Agree To Respawn As Ghost When Die?^n")
...
{ 
	...
	if(g_iCachepCvar[4])
	{
		...
		new origins[3]
		origins[0] = floatround(g_fGhostPosition[id][0])
		origins[1] = floatround(g_fGhostPosition[id][1])
		origins[2] = floatround(g_fGhostPosition[id][2])
