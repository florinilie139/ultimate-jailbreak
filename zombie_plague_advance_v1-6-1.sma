...
public plugin_init()
{
	...
	if (g_gamemodes_i > MAX_GAME_MODES)
		server_print("[Zombilitza] Total Registered Custom Game Modes: %d", g_gamemodes_i - MAX_GAME_MODES)
	
	// Print the number of registered Zombie Classes
	server_print("[Zombilitza] Total Registered Zombie Classes: %d", g_zclass_i)
	
	// Print the number of registered Extra Items
	server_print("[Zombilitza] Total Registered Extra Items: %d", g_extraitem_i)
...
public clcmd_buyammo(id)
{
	...
	if (g_zombie[id])
	{
		zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "CMD_HUMAN_ONLY")
		return PLUGIN_HANDLED;
	}
	...
	if (g_ammopacks[id] < 1)
	{
		zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "NOT_ENOUGH_AMMO")
		return PLUGIN_HANDLED;
	}
	...
	zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "AMMO_BOUGHT")
...
show_menu_extras(id)
{
	...
	{
		zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id ,"CMD_NOT_EXTRAS")
		menu_destroy(menuid)
		return;
	}
...
public show_menu_game_mode(id)
{
	...
	{
		// Print a message
		zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "CMD_NOT_GAME_MODES")
	...
	switch (key)
	{
		case 0: // Buy Weapons
		{
			...
			if (get_pcvar_num(cvar_buycustom))
			{
				...
				WPN_AUTO_ON = 0
				zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "BUY_ENABLED")
				
				// Show menu if player hasn't yet bought anything
				if (g_canbuy[id]) show_menu_buy1(id)
			}
			else
				zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "CMD_NOT")
		}
		case 1: // Extra Items
		{
			// Extra items enabled?
		...
		case 2: // Zombie Classes
		{
			...
			if (get_pcvar_num(cvar_zclasses))
				...
				zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "CMD_NOT_ZCLASSES")
		}
		case 3: // Unstuck
		{
			...
			if (g_isalive[id])
			{
				if (is_player_stuck(id))
				{
					...
					zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "CMD_NOT_STUCK")
			}
			else
				zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "CMD_NOT")
		}
		...
		case 8: // Admin Menu
		{
			...
			if (get_user_flags(id) & g_access_flag[ACCESS_ADMIN_MENU3])
				...
				zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "CMD_NOT_ACCESS")
		}
	}
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	{
		zp_colored_print(id, "^x04[Zombilitza]^x01 %L", id, "CMD_NOT_CANTUSE")
		return;
	}
...
public menu_zclass(id, menuid, item)
{
	...
	zp_colored_print(id, "^x04[Zombilitza]^x01 %L^x01:^x04 %s", id, "ZOMBIE_SELECT", name)
	zp_colored_print(id, "^x04[Zombilitza]^x01 %L^x01:^x04 %d^x01 |^x01 %L^x01:^x04 %d^x01 |^x01 %L^x01:^x04 %d^x01 |^x01 %L^x01:^x04 %d%%", id, "ZOMBIE_ATTRIB1", ArrayGetCell(g_zclass_hp, g_zombieclassnext[id]), id, "ZOMBIE_ATTRIB2", ArrayGetCell(g_zclass_spd, g_zombieclassnext[id]),
...
public cmd_zombie(id, level, cid)
{
	...
	{
		client_print(id, print_console, "[Zombilitza] %L", id, "CMD_NOT")
		return PLUGIN_HANDLED
	}
...
public welcome_msg()
{
	...
	zp_colored_print(0, "^x04[Zombilitza]^x01 %L", LANG_PLAYER, "NOTICE_INFO1")
	if (!get_pcvar_num(cvar_infammo)) zp_colored_print(0, "^x04[Zombilitza]^x01 %L", LANG_PLAYER, "NOTICE_INFO2")
...
check_round(leaving_player)
{
	...
	if (g_zombie[leaving_player] && fnGetZombies() == 1)
	{
		...
		zp_colored_print(0, "^x04[Zombilitza]^x01 %L", LANG_PLAYER, "LAST_ZOMBIE_LEFT", g_playername[id])
	...
	else if (!g_zombie[leaving_player] && fnGetHumans() == 1)
	{
		...
		zp_colored_print(0, "^x04[Zombilitza]^x01 %L", LANG_PLAYER, "LAST_HUMAN_LEFT", g_playername[id])
