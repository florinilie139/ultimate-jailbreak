...
static const PT_TAG[]    = "[Jb.LaLeagane.Ro]"
...
public player_teleport(id) {
    ...
    if(get_user_flags(id) & ADMIN_LEVEL_E)
    {
        ...
        if(!is_user_alive(id)) {
            ColorChat(id,RED,"^x04%s^x03 Nu poti folosi aceasta comanda cand esti mort !",PT_TAG)
            return PLUGIN_HANDLED;
        }
    ...
    {
        ColorChat(id,RED,"^x04%s^x03 Nu esti destul de boss ca sa folosesti comanda asta !",PT_TAG)
        return PLUGIN_HANDLED;
    }
    return PLUGIN_HANDLED;    
}