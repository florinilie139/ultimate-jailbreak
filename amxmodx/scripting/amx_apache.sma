...
new rocketspeed,distance,height,beams,cost,admins,ff;
...
public create_apache(id,level,cid)
{
	...
	if(get_pcvar_num(admins) == 1)
	{
		if(!(get_user_flags(id) & APACHE_ADMIN))
		{
	...
	if(apaches[id] > 0)
	{
		...
		client_print(id, print_center, "You already control a helicopter.")
		return PLUGIN_HANDLED
	}
	...
	if(!is_user_alive(id))
	{
		...
		client_print(id, print_center, "You cannot control a helicopter when you are dead.")
		return PLUGIN_HANDLED
	}
	...
	if(!(get_user_flags(id) & APACHE_ADMIN))
	{
		...
		if(usermoney < apachecost)
		{
			...
			client_print(id, print_center, "You don't have enough money ($%i needed)", apachecost)
			return PLUGIN_HANDLED
		}
